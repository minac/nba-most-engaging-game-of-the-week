...
def setup_test_environment():
    ...
    yield
    # Cleanup test database directory if it exists
    test_data_dir = Path(project_root) / "data"
    if test_data_dir.exists():
        for db_file in test_data_dir.glob("test_*.db"):
            db_file.unlink()
...
def mock_nba_client_with_cache_disabled(star_players, top5_teams):
    ...
    with patch("src.api.nba_api_client.NBAClient") as MockClient:
        mock_client = Mock()
